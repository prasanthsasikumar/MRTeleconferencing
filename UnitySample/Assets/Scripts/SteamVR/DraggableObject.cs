...
public class DraggableObject : MonoBehaviour
{
    ...
    public void Update()
    {
        if (controllerTransform != null)
            print(controllerTransform.position);
    }
    ...
    private void FixedUpdate()
    {
        if (controllerTransform != null)
        {
            if (lastPosition.HasValue)
            {
                ...
                Vector3 angularVelocity = Vector3.zero;// SteamVR_Controller.Input((int)controllerTransform.GetComponent<SteamVR_TrackedObject>().index).angularVelocity;
    ...
    protected virtual void UpdatePosition(Vector3 targetPosition, float followingSpeed)
    {
        ...
        print("Offset :" + zPositionOffset);
        if (controllerTransform.position.z + zPositionOffset < wallTransform.position.z)//if your pen sticks to whiteboard, change '<' to '>'
